...
const SmallParcels = () => {
  ...
  const fetchBranches = async () => {
    try {
      setIsLoading(true);
      const response = await fetch('https://localhost:1000/api/listBranch', {
        method: 'POST',
        headers: {
  ...
  const checkBarcodeAndAdd = async () => {
    ...
    try {
      ...
      const response = await fetch(`https://localhost:1000/api/smallParcels`, {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
        },
  ...
  const handleCreateShipment = async () => {
    ...
    try {
      ...
      // ส่งข้อมูลไปยัง API
      const response = await fetch('https://localhost:1000/api/smallParcelsSave', {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
        },
